export function amimals(hues) {

  const rn = (n) => { return Math.floor(Math.random() * n); }
  const N = 6; // subdivisions of canvas (N x N grid); change to make grid more/less dense
  ...
  async function shape(z, i, j, k, l) { // z: lg or sm (without or with subunits); i,j,k,l are counters from calling function, used as multiples
    ...
    let r = rn(4); // rotation
    let s = rn(11); // shape, or default no shape
    ...
    //   case 10: svgMod("paw");; break; // pawprint
    //   default: break;
    // }
    svgMod("paw");
    ...
    // HELPER FUNCTION - MOD INLINE SVG IMAGES PRIOR TO DRAWING
    function svgMod(ele) {
      let svgURL = new XMLSerializer().serializeToString(document.getElementById(ele)).replace("blanchedalmond", fg);
      let img = new Image();
      ...
      img.onload = function () {
        console.log("drw");
        // cx.drawImage(this, 0, 0, d, d);
        ctx.drawImage(this, 0, 0, mw / 3, mw / 3, dx + i * u + k * u / 3 - .5, dx + j * u + l * u / 3 - .5, mw / 3 + .5, mw / 3 + .5);
        // cx.restore();
      }
    }
  }
